// Format helper
    ...
    function calculate() {
      ...
      // Save key values for Takaful page
      localStorage.setItem('totalSalary', String(income));
      localStorage.setItem('totalExpenses', String(expenses));
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      if (showTakafulBtn && takafulSection) {
        showTakafulBtn.addEventListener('click', function () {
          ...
          document.getElementById('salaryText').textContent = salary > 0 ? `RM ${salary.toLocaleString()}` : 'No data found';
          document.getElementById('expensesText').textContent = expenses > 0 ? `RM ${expenses.toLocaleString()}` : 'No data found';
          document.getElementById('netText').textContent = (salary > 0 && expenses > 0) ? `RM ${(salary - expenses).toLocaleString()}` : 'No data found';
        });
      }
    ...
    function showCoverage(plan) {
      ...
      if (salary <= 0 || expenses <= 0) {
        ...
        document.getElementById('resultSectionTakaful').classList.remove('animate-fadeIn');
        return;
      }
